...
int main(int argc, char* argv[])
{
    ...
    catch (ParserError& e) {   //parser error
      cout << e.what() << endl;
      return 1;
    }
    catch (std::exception& e) {   //any other error with parser
    ...
    while(option != 0) {
        ...
        catch (UserNotLoggedInError& e) {  //if user not logged in
          cout << no_user_except_msg << endl;
        }
        catch (RatingLimitError& e) {  //if the rating of the content is too high
          cout << rating_limit_except_msg << endl;
        }
        catch (ReviewRangeError& e) {  //if the review is not within range 0-5
